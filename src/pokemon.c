...
u16 GetEvolutionTargetSpecies(struct Pokemon *mon, u8 mode, u16 evolutionItem, struct Pokemon *tradePartner)
{
    ...
    if ((holdEffect == HOLD_EFFECT_PREVENT_EVOLVE || GetMonData(mon, MON_DATA_EFFORT_RIBBON, NULL))
...
bool8 TryIncrementMonLevel(struct Pokemon *mon)
{
    ...
    if (nextLevel > GetLevelCeiling() || expPoints < gExperienceTables[gSpeciesInfo[species].growthRate][nextLevel])
    {
        return FALSE;
    }
    else
...
u16 GetBattleBGM(void)
{
    ...
    else if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
    {
        ...
        case TRAINER_CLASS_CREEPING:    
            return MUS_VS_GIRATINA;
        default:
            return MUS_VS_TRAINER;
        }
    }
