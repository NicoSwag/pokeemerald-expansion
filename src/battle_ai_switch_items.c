...
static bool32 HasBadOdds(u32 battler, bool32 emitResult)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        if (aiMove != MOVE_NONE)
        {
            ...
            || aiMoveEffect == EFFECT_EMP
            || aiMoveEffect == EFFECT_DIRTY_BOMB
    ...
    for (i = firstId; i < lastId; i++)
    {
        ...
        for (j = 0; j < MAX_MON_MOVES; j++)
        {
            ...
            if (aiMove != MOVE_NONE && gMovesInfo[aiMove].power != 0)
            {
                ...
                if ((gMovesInfo[aiMove].effect == EFFECT_EXPLOSION || gMovesInfo[aiMove].effect == EFFECT_EMP || gMovesInfo[aiMove].effect == EFFECT_DIRTY_BOMB) && damageDealt < playerMonHP)
                    continue;
