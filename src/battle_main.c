...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    TRAINER_CLASS(COOLTRAINER, "Ace Trainer", 12, ITEM_ULTRA_BALL),
    ...
    TRAINER_CLASS(PICNICKER, "Picknicker", 4),
    ...
    TRAINER_CLASS(SR_AND_JR, "Sr. & Jr.", 4),
    ...
    TRAINER_CLASS(TRIATHLETE, "Thriathlete", 10),
    ...
    TRAINER_CLASS(ENGINEER, "Devon", 5, ITEM_HEAVY_BALL),
    TRAINER_CLASS(SCIENTIST, "Scientist"),
    TRAINER_CLASS(MINER, "Miner"),
    TRAINER_CLASS(SCAMMERS, "Scammer", 10),
    TRAINER_CLASS(COP, "Security", 5, ITEM_DUSK_BALL),
};
...
const u8* FaintClearSetData(u32 battler)
{
    ...
    gProtectStructs[battler].electroShielded = FALSE;
        ...
        if (gStatuses3[otherSkyDropper] & STATUS3_SKY_DROPPED)
        {
            ...
            if (gBattleMons[otherSkyDropper].status2 & STATUS2_LOCK_CONFUSE)
            {
                ...
                if (!(GetBattlerAbility(otherSkyDropper) == ABILITY_OWN_TEMPO
                    ...
                    || gBattleMons[otherSkyDropper].types[0] == TYPE_BUG
                    || gBattleMons[otherSkyDropper].types[1] == TYPE_BUG))
                {
...
u32 GetBattlerTotalSpeedStatArgs(u32 battler, u32 ability, u32 holdEffect)
{
    ...
    if (WEATHER_HAS_EFFECT)
    {
        if (ability == ABILITY_SWIFT_SWIM       && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && gBattleWeather & B_WEATHER_RAIN)
            ...
            speed *= 2;
        else if (ability == ABILITY_FOUL_RUSH  && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && (gBattleWeather & (B_WEATHER_POLLUTION)))
            speed *= 2;
    }
...
s8 GetMovePriority(u32 battler, u16 move)
{
    ...
    && gBattleMons[battler].hp >= gBattleMons[battler].maxHP * 0.75
    && gMovesInfo[move].type == TYPE_WATER
    ){
    priority++;
    }
    else if (ability == ABILITY_PRANKSTER && IS_MOVE_STATUS(move))
    {
        ...
        else if (ability == ABILITY_TRICKSTER && IsBattlerTerrainAffected(battler,STATUS_FIELD_MISTY_TERRAIN) && gMovesInfo[move].type == TYPE_FAIRY && (gBattleMons[battler].hp >= gBattleMons[battler].maxHP * 0.75))
    {
...
void SetTypeBeforeUsingMove(u32 move, u32 battlerAtk)
{
    ...
    if (gMovesInfo[move].effect == EFFECT_WEATHER_BALL)
    {
        if (WEATHER_HAS_EFFECT)
        {
            ...
            else if (gBattleWeather & (B_WEATHER_POLLUTION) & holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA)
