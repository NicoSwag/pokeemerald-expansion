...
static void Cmd_attackcanceler(void)
{
    ...
    else if (gSpecialStatuses[gBattlerTarget].steamBarrierRedirected)
    {
        gSpecialStatuses[gBattlerTarget].steamBarrierRedirected = FALSE;
        gLastUsedAbility = ABILITY_STEAM_BARRIER;
        BattleScriptPushCursor();
        gBattlescriptCurrInstr = BattleScript_TookAttack;
        RecordAbilityBattle(gBattlerTarget, gLastUsedAbility);
    }
     ...
     && gMovesInfo[gCurrentMove].effect != EFFECT_UPPER_HAND)
    {
        ...
        if (gSpecialStatuses[gBattlerAttacker].strikerState == STRIKER_1ST_HIT)
        {
         ...
         if (gSpecialStatuses[gBattlerAttacker].fistBarrageState == FIST_BARRAGE_1ST_HIT)
        {
            gSpecialStatuses[gBattlerAttacker].fistBarrageState = FIST_BARRAGE_OFF; // No second hit if first hit was blocked
            gSpecialStatuses[gBattlerAttacker].multiHitOn = 0;
            gMultiHitCounter = 0;
        }
...
static bool32 AccuracyCalcHelper(u16 move)
{
    ...
    else if (GetBattlerAbility(gBattlerAttacker) == ABILITY_FIXED_GAZE && (move != MOVE_SKY_DROP || gBattleStruct->skyDropTargets[gBattlerTarget] == 0xFF))
    {
        if (!JumpIfMoveFailed(7, move))
            RecordAbilityBattle(gBattlerAttacker, ABILITY_FIXED_GAZE);
        return TRUE;
    }
    else if (GetBattlerAbility(gBattlerAttacker) == ABILITY_PAROTOID_GLANDS && (move != MOVE_SKY_DROP || gBattleStruct->skyDropTargets[gBattlerTarget] == 0xFF) && gMovesInfo[move].type==TYPE_POISON)
    {
        if (!JumpIfMoveFailed(7, move))
            RecordAbilityBattle(gBattlerAttacker, ABILITY_PAROTOID_GLANDS);
        return TRUE;
    }
    ...
    else if (GetBattlerAbility(gBattlerTarget) == ABILITY_NO_GUARD && (move != MOVE_SKY_DROP || gBattleStruct->skyDropTargets[gBattlerTarget] == 0xFF))
    {
       ...
       else if (GetBattlerAbility(gBattlerTarget) == ABILITY_FAE_FORCE && (move != MOVE_SKY_DROP || gBattleStruct->skyDropTargets[gBattlerTarget] == 0xFF) && IsBattlerTerrainAffected(gBattlerAttacker,STATUS_FIELD_MISTY_TERRAIN) && gMovesInfo[move].type == TYPE_FAIRY)
    {
        if (!JumpIfMoveFailed(7, move))
...
static void Cmd_attackanimation(void)
{
    ...
    else
    {
        ...
        if ((gSpecialStatuses[gBattlerAttacker].fistBarrageState == FIST_BARRAGE_2ND_HIT) || (gSpecialStatuses[gBattlerAttacker].fistBarrageState == FIST_BARRAGE_3RD_HIT) || (gSpecialStatuses[gBattlerAttacker].fistBarrageState == FIST_BARRAGE_4TH_HIT))// No animation on second hit
        {
            gBattlescriptCurrInstr = cmd->nextInstr;
            return;
        }
        ...
        else
        {
            ...
            case MOVE_EFFECT_FLINCH:
                if (battlerAbility == ABILITY_INNER_FOCUS)
                {
                    ...
                    else if (gBattleMons[gEffectBattler].types[0] == TYPE_PSYCHIC
                            || gBattleMons[gEffectBattler].types[1] == TYPE_PSYCHIC)
                    {
                        gBattlescriptCurrInstr = BattleScript_MoveEnd;
                    }
            ...
            case MOVE_EFFECT_FEINT:
                if (IS_BATTLER_PROTECTED(gBattlerTarget))
                {
                    ...
                    gProtectStructs[gBattlerAttacker].electroShielded = FALSE;
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
...
static void Cmd_cleareffectsonfaint(void)
{
    ...
    if (gBattleControllerExecFlags == 0)
    {
        ...
        if(gBattleMons[battler].ability==ABILITY_PSYCHIC_SURGE && gBattleMons[battler].canTerrainChange == TRUE && STATUS_FIELD_PSYCHIC_TERRAIN){
            DrawMainBattleBackground();
            gFieldStatuses &= ~STATUS_FIELD_PSYCHIC_TERRAIN;
        }
        if(gBattleMons[battler].ability==ABILITY_ELECTRIC_SURGE && gBattleMons[battler].canTerrainChange == TRUE && STATUS_FIELD_ELECTRIC_TERRAIN){
            DrawMainBattleBackground();
            gFieldStatuses &= ~STATUS_FIELD_ELECTRIC_TERRAIN;
        }
        if(gBattleMons[battler].ability==ABILITY_MISTY_SURGE && gBattleMons[battler].canTerrainChange == TRUE && STATUS_FIELD_MISTY_TERRAIN){
            DrawMainBattleBackground();
    ...
    do
    {
        ...
        case MOVEEND_PROTECT_LIKE_EFFECT:
            if (gProtectStructs[gBattlerAttacker].touchedProtectLike)
            {
                ...
                else if (gProtectStructs[gBattlerTarget].electroShielded)
                {
                    gProtectStructs[gBattlerAttacker].touchedProtectLike = FALSE;
                    gBattleScripting.moveEffect = MOVE_EFFECT_PARALYSIS | MOVE_EFFECT_AFFECTS_USER;
                    PREPARE_MOVE_BUFFER(gBattleTextBuff1, MOVE_ELECTROSHIELD);
                    BattleScriptPushCursor();
                    gBattlescriptCurrInstr = BattleScript_BanefulBunkerEffect;
                    effect = 1;
                }
        ...
        case MOVEEND_MOVE_EFFECTS2: // For effects which should happen after target items, for example Knock Off after damage from Rocky Helmet.
        {
            ...
            && gMovesInfo[gChosenMoveByBattler[BATTLE_PARTNER(gBattlerAttacker)]].target != (MOVE_TARGET_ALL_BATTLERS || MOVE_TARGET_ALLY || MOVE_TARGET_BOTH || MOVE_TARGET_FOES_AND_ALLY || MOVE_TARGET_OPPONENTS_FIELD || MOVE_TARGET_RANDOM)
            ){
            ...
            && !(gMovesInfo[gCurrentMove].effect == EFFECT_PRESENT && gBattleStruct->presentBasePower == 0)) // Silly edge case
            {
                ...
                else
                {
                    ...
                    && !(gBattleMons[gBattlerAttacker].status1 & STATUS1_FREEZE))
                    {
                        ...
                        if (gSpecialStatuses[gBattlerAttacker].fistBarrageState)
                            gSpecialStatuses[gBattlerAttacker].fistBarrageState--;
        ...
        case MOVEEND_SINGER: // Special case because it's so annoying
            if (gMovesInfo[gCurrentMove].soundMove )
            {
                ...
                if (!(gBattleStruct->lastMoveFailed & gBitTable[gBattlerAttacker]
                    || (!gSpecialStatuses[gBattlerAttacker].singerUsedMove
                        && gProtectStructs[gBattlerAttacker].usesBouncedMove)))
                {   // Dance move succeeds
                    // Set target for other Dancer mons; set bit so that mon cannot activate Dancer off of its own move
                    if (!gSpecialStatuses[gBattlerAttacker].singerUsedMove)
                    ...
                    if (nextSinger && AbilityBattleEffects(ABILITYEFFECT_MOVE_END_OTHER, nextSinger & 0x3, 0, 0, 0))
                        effect = TRUE;
                }
            }
            gBattleScripting.moveendState++;
...
static void RemoveAllWeather(void)
{
    ...
    if (gBattleWeather & B_WEATHER_RAIN)
        ...
        gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_WEATHER_END_POLLUTION;
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_TRY_ACTIVATE_COMET_PUNCH:
    {
        VARIOUS_ARGS();
        if (gMovesInfo[gCurrentMove].effect == EFFECT_COMET_PUNCH
            && HasAttackerFaintedTarget()
            && !NoAliveMonsForEitherParty()
            && gSpecialStatuses[gBattlerAttacker].fistBarrageState != FIST_BARRAGE_4TH_HIT
            && CompareStat(gBattlerAttacker, STAT_ATK, MAX_STAT_STAGE, CMP_LESS_THAN))
        {
            SET_STATCHANGER(STAT_ATK, 1, FALSE);
            PREPARE_STAT_BUFFER(gBattleTextBuff1, STAT_ATK);
            BattleScriptPush(cmd->nextInstr);
            gBattlescriptCurrInstr = BattleScript_CometPunchRaisesStat;
            return;
        }
        break;
    }
...
static void Cmd_setprotectlike(void)
{
        ...
        || (gCurrentMove == MOVE_QUICK_GUARD && B_QUICK_GUARD != GEN_5))
    {
        if (!gMovesInfo[gCurrentMove].argument) // Protects one mon only.
        {
            ...
            else if (gCurrentMove == MOVE_ELECTROSHIELD)
            {
                gProtectStructs[gBattlerAttacker].electroShielded= TRUE;
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_PROTECTED_ITSELF;
            }
...
static void Cmd_trysetencore(void)
{
    ...
    if (gBattleMons[gBattlerAttacker].types[0] == TYPE_FIGHTING || gBattleMons[gBattlerAttacker].types[1] == TYPE_FIGHTING)
        {
            gBattlescriptCurrInstr = cmd->nextInstr;
...
void BS_CheckFistBarrageCounter(void)
{
    NATIVE_ARGS(u8 counter, const u8 *jumpInstr);
    // Some effects should only happen on the first or second strike of Parental Bond,
    // so a way to check this in battle scripts is useful
...
void BS_SetRemoveTerrain(void)
{
    ...
    else
    {
        ...
        gFieldStatuses |= statusFlag;
        if(atkHoldEffect == HOLD_EFFECT_TERRAIN_EXTENDER )
